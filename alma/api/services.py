...
def run_plan_main(
    ...
    graph_path, game, patrol = _resolve_inputs(payload)
...
def run_plan_efficiency(
    ...
    for index, units in enumerate(units_list, start=1):
        report(0.05 + 0.9 * (index / len(units_list)), f"Comparing {units} patrol unit(s)...")
...
def run_plan(
    ...
    return run_plan_main(payload, progress=progress)
